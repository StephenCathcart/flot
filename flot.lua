...
<html>
 <head>
    ...
    <!--[if lte IE 8]><script language="javascript" type="text/javascript" src="%shttps://cdnjs.cloudflare.com/ajax/libs/flot/0.8.3/excanvas.min.js"></script><![endif]-->
    <script language="javascript" type="text/javascript" src="%shttps://cdnjs.cloudflare.com/ajax/libs/jquery/3.4.0/jquery.min.js"></script>
    <script language="javascript" type="text/javascript" src="%shttps://cdnjs.cloudflare.com/ajax/libs/flot/0.8.3/jquery.flot.js"></script>
    <script language="javascript" type="text/javascript" src="%shttps://cdnjs.cloudflare.com/ajax/libs/flot.tooltip/0.9.0/jquery.flot.tooltip.min.js"></script>
 </head>
    <body>
]]
...
flot.null = setmetatable({}, {__tostring = function(self) return 'null' end})
...
function as_js(t)
    ...
    elseif #t > 0 then -- it's an array!
        local res = {}
        for i = 1, #t do res[i] = as_js(t[i]) end
        return '[' .. concat(res, ',') .. ']'
    else
        local res = {}
        for k, v in pairs(t) do append(res, k .. ':' .. as_js(v)) end
        return '{' .. concat(res, ',') .. '}'
    end
end
...
local function interleave(xv, yv)
    local res = {}
    for i = 1, #xv do res[i] = {xv[i], yv[i]} end
    return res
end
...
function flot.Plot(opts)
    ...
    function plot:script() return script:format(self.idx, as_js(dataset), as_js(opts)) end
...
function flot.render(plot)
    local p = flot.path
    local header = prefix:format(p, p, p, p)
